				...
				switch((unsigned char)command){
					...
					case 33:	// arm info
						if(!additional_data_in_buffer){
							...
							arm_info.joints = (unsigned int)read_buffer[2];
							...
							//if(!in_state.connection_started)
...
int SerialPortRobotArm::moveJointTo(int joint, float speed, float pos){
	...
	tmp_uint = (unsigned int)(pos*100.0f);

	write_buffer[5] = (unsigned char)(tmp_uint>>8);
	write_buffer[6] = (unsigned char)(tmp_uint);
	
    DEBUGMSG("Moving joint %d to position %f with speed %f\n", joint, pos, speed);
