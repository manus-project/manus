...
MACRO(EMBED_GENERATE)
    ...
    FOREACH(EMBED_FILE ${EMBED_FILES})
        ...
        FILE(APPEND "${RESOURCE_FILE}" " \"${EMBED_PREFIX}${EMBED_NAME}\", \n")
        SET(RESOURCE_POSITION 0)
        STRING(LENGTH "${CONTENT}" RESOURCE_LENGTH)
        WHILE (RESOURCE_POSITION LESS RESOURCE_LENGTH)
            MATH(EXPR RESOURCE_PART "${RESOURCE_LENGTH} - ${RESOURCE_POSITION}")
            IF (RESOURCE_PART GREATER 15999)
                SET(RESOURCE_PART 16000)
            ENDIF()
            STRING(SUBSTRING "${CONTENT}" ${RESOURCE_POSITION} ${RESOURCE_PART} PARTIAL_CONTENT)
            FILE(APPEND "${RESOURCE_FILE}" " \"${PARTIAL_CONTENT}\"")
            MATH(EXPR RESOURCE_POSITION "${RESOURCE_POSITION} + ${RESOURCE_PART}" )
        ENDWHILE()
